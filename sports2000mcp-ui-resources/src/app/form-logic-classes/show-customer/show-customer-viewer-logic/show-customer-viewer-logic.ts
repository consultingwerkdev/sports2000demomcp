...
import { CUSTOMER_UPDATED_EVENT } from './show-customer-viewer.constants';

/**
 * Sends customer save events from the Smart viewer into MCP model context.
 *
 * @memberof Sports2000McpShowCustomerViewerLogic
 */
...
export class Sports2000McpShowCustomerViewerLogic extends SmartViewerLogic {
  ...
  /**
   * Resolves the MCP bridge before the viewer lifecycle continues.
   *
   * @memberof Sports2000McpShowCustomerViewerLogic
   */
  override init(): void {
  ...
  /**
   * Publishes the saved customer record to MCP model context after the viewer saves changes.
   *
   * @param {AfterSaveChangesEventArgument} args - The Smart viewer save event payload.
   * @memberof Sports2000McpShowCustomerViewerLogic
   */
